...
def main():
    parser = argparse.ArgumentParser(
        add_help=True,
        description="Pixelate Images, " "returns json " "output of pixel locations",
    )
    parser.add_argument("i", nargs="?", type=Path, help="input image file path")
    parser.add_argument(
        "--o", required=False, type=str, help="path to write json output"
    )
    parser.add_argument(
        "--s",
        default=(50, 50),
    ...
    parser.add_argument(
        "--r", default=0, type=int, help="int degree of rotation for image"
    )
    parser.add_argument(
        "--t", type=int, help="int 0-255 threshold for image binerization"
    )
    parser.add_argument(
        "--json", help="returns json of points", action="store_true", default=False
    )
